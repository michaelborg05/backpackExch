        ...
        for profile_name in alert.profiles:
            try:
                ...
                if profile.use_market_regime_filter:
                    ...
                    primary_timeframe=trend_timeframe,
                    confirm_timeframe=getattr(profile, 'entry_timeframe', '15'),
...
# ---------------------------------------------------------------------------
# GET /profiles/export-all-python  (must be before /{profile_name} catch-all)
# ---------------------------------------------------------------------------
...
async def create_profile_endpoint(body: ProfileCreateRequest, current_user=Depends(get_dashboard_user)):
    ...
    with get_db_session() as db:
        ...
        p = TradingProfileDB(
            ...
            max_cluster_entries=body.max_cluster_entries,
...
async def update_profile_endpoint(profile_name: str, body: ProfileUpdateRequest):
    ...
    _EDITABLE = [
        ...
        "signal_cooldown_minutes", "sl_cooldown_minutes", "tp_cooldown_minutes", "max_cluster_entries",
...
def _serialize_profile(p, cb=None) -> dict:
    ...
    return {
        ...
        "max_cluster_entries":         p.max_cluster_entries,
