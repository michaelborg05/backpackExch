        ...
        for indicator_config in indicators:
            ...
            if indicator_type == "ema_cross":
                ...
                if slope_pct is None:
                    ...
                    elif required_direction == "not_rising":
                        is_bullish = slope_pct <= max_slope_pct if max_slope_pct is not None else slope_pct <= min_slope_pct
                        direction = "flat/falling" if is_bullish else "rising"
                    else:
                ...
                Long (default, side="long"):
                  ...
                  With lookback_candles: also blocks if RSI exceeded min_value in
                  the last N candles (catches post-peak cooling).

                Short (side="short"):
                  Blocks if current RSI < max_value (oversold — bad time to short).
                  params: { side: "short", max_value: 30, lookback_candles: null }
                  With lookback_candles: also blocks if RSI dropped below max_value
                  in the last N candles (catches post-trough bounce entries).
                ...
                if side == "short":
                    # Short: block when RSI is oversold (too low to enter a short)
                    ...
                    is_bullish = not is_extreme and not trough_in_lookback
                    ...
                    # Long (default): block when RSI is overbought (too high to enter a long)
                ...
                # Bullish when price sits between min/max % below the rolling
                # high over the lookback window — a "how far off the recent
                # high" dip filter, independent of any EMA. Default is the
                # validated 7-day lookback (42 4h bars); the original 30-day/
                # 180-bar version was a single-symbol concentration artifact
                # (see backtesting/profile_samples/dip_buy_variants.py).
                # Caller must keep lookback_bars <= the cache's retention cap
                # (see the "Keep last N candles" truncation in feed() above)
                # or this silently uses fewer bars than requested — the msg
                # below reports the bar count actually used so a mismatch is
